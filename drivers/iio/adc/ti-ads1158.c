...
/* FIXME: add reg_read to fix the reading SPI reset issue. */
...
{
	unsigned int tmp = clock_frequency / clock_cycles;

	return DIV_ROUND_UP(MEGA, tmp);
}
				     ...
				     struct iio_chan_spec const *chan, int *val)
{
	...
	conversion_time_us = clock_cycles_to_us(st->clock_frequency, buf);
...
release:
	...
	*val = (__s16)be16_to_cpu((__be16)(rx_buf[1]));
